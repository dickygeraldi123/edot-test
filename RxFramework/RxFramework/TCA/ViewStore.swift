...
//        upstream.observe(on: MainScheduler.instance).subscribe(observer)
        return upstream
    ...
    where LocalState: Equatable {
        .init(store: store, upstream: self.upstream.map { $0[keyPath: keyPath] }.distinctUntilChanged())
    }
}
...
extension ViewStore {
    public func subscribeNeverEqual<LocalState: Equatable>(
        _ toLocalState: @escaping (ViewState) -> NeverEqual<LocalState>
    ) -> Effect<LocalState> {
        _state.map(toLocalState).distinctUntilChanged()
            .map(\.wrappedValue)
            .eraseToEffect()
    }
}
...
extension ViewStore where ViewState: Equatable {
    public func subscribe() -> Effect<ViewState> {
        return _state.distinctUntilChanged().eraseToEffect()
    }
}
