...
public final class Store<State, Action> {
    ...
    public func scope<ChildState, ChildAction>(
        ...
        #if swift(>=5.7)
        return self.reducer.rescope(self, state: toChildState, action: { fromChildAction($1) }, removeDuplicates: isDuplicate)
        #else
    ...
    public func scope<ChildState>(
        ...
        self.scope(state: toChildState, action: { $0 }, removeDuplicates: nil)
    }
    ...
    func filter(
        ...
        #if swift(>=5.7)
        return self.reducer.rescope(self, state: { $0 }, action: { isSent($0, $1) ? $1 : nil }, removeDuplicates: nil)
        #else
                return (self.scope ?? StoreScope(root: self))
    ...
    func invalidate(_ isInvalid: @escaping (State) -> Bool) -> Store {
        ...
        store._isInvalidated = { self._isInvalidated() || isInvalid(self.state) }
        return store
    }
    ...
    public var actionless: Store<State, Never> {
        ...
        return self.scope(state: { $0 }, action: absurd, removeDuplicates: nil)
    }
